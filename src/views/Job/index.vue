<template>
  <div class="job-page">
    ...
    <el-table id="selectTable" :data="staffJobData" border style="width: 100%" @selection-change="handleSelectionChange">
...
<script>
import { getAllStaffJobRecord, getStaffJobInfoByKeyWord, updateStaffJobInfoByCode, jobDel } from '@/api';
...
export default {
  ...
  methods: {
    ...
    del(item) {
      ...
      this.$confirm('此操作将永久删除该员工的调岗信息, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        jobDel(item).then(res => {
          console.log(res);
          this.$message({
            type: 'success',
            message: '删除成功!'
    ...
    search() {
      if (this.keyWord === '' || this.keyWord === null) {
        ...
        getStaffJobInfoByKeyWord({ keyWord: this.keyWord }).then(res => {
          this.staffJobData = [];
