...
<script>
import { getAllStaffVacateRecord, getStaffVacateInfoByKeyWord, updateStaffVacateInfoByCode, delVacate } from '@/api';
...
export default {
  ...
  methods: {
    ...
    del(item) {
      ...
      this.$confirm('此操作将永久删除该员工的请假记录, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        delVacate(item).then(res => {
          console.log(res);
          this.$message({
            type: 'success',
            message: '删除成功!'
